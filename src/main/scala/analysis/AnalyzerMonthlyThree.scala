...
object AnalyzerMonthlyThree extends App {
    ...
    for (inMintue <- (0 to 57) if inMintue % mintueHind == 0) {
      for (outHour <- Seq(12, 13) if inHour < outHour) {
        for (outMintue <- (0 to 57) if outMintue % mintueHind == 0) {
          for (endHour <- Seq(14, 15) if outHour < endHour) {
                  ...
                  for (hosei <- -20 to 0 if hosei % 10 == 0) {
                    for (max <- 200 to 300 if max % 10 == 0) {
                      if (endHour != 15 || endMintue <= 15) {
                        //strategyList +:= new StrategyThree(inHour, inMintue, outHour, outMintue, endHour, endMintue, rieki, son, hosei, max)
                      }
                    }
                  }
                }
  ...
  strategyList +:= new StrategyThree(10, 0, 12, 45, 14, 0, 190, 90, -20, 280)
