...
class StrategyOne(inHour: Int, inMintue: Int, outHour: Int, outMintue: Int, endHour: Int, endMintue: Int, closingHour: Int, closingMintue: Int, rieki: Int, son: Int, hosei: Int, max: Int) {
  ...
  val CLOSING_HOUR = closingHour
  val CLOSING_MINTUE = closingMintue
  val RIEKI = rieki
  val SON = son
  val HOSEI = hosei
  val MAX = max
  ...
  def params: String = {
    s"$IN_HOUR,$IN_MINTUE,$OUT_HOUR,$OUT_MINTUE,$END_HOUR,$END_MINTUE,$CLOSING_HOUR,$CLOSING_MINTUE,$RIEKI,$SON,$HOSEI,$MAX"
  }
  ...
  def isClosingTime(tradeTime: LocalTime): Boolean = {

    val hour = tradeTime.getHourOfDay
    if (hour > CLOSING_HOUR || (hour == CLOSING_HOUR && tradeTime.getMinuteOfHour >= CLOSING_MINTUE)) true
    else false
  }

  def isEndTime(hour: Int, mintue: Int): Boolean = {

    if (hour > END_HOUR || (hour == END_HOUR && mintue >= END_MINTUE)) true
    else false
  }
        ...
        if (isTradeTime(hour, minute)) {
          ...
          if (pointHigh - pointLow <= MAX) {
            if (isHighTouch == false && pointHigh + HOSEI < price.price) {
              ...
              if (!isLowTouch) {
                if (isEndTime(hour, minute) == false) buyOrderIndexs +:= new OrderInfo(i, rieki, son)
              }
              isHighTouch = true
            } else if (isLowTouch == false && pointLow - HOSEI > price.price) {
              val (rieki, son) = getRiekiSon(pointHigh, pointLow)
              if (!isHighTouch) {
                if (isEndTime(hour, minute) == false) sellOrderIndexs +:= new OrderInfo(i, rieki, son)
              }
              isLowTouch = true
            }
          } else {
      ...
      b.breakable {
        for (price <- priceList.drop(buyOrder.index + 1)) {
          if (order.price + buyOrder.rieki < price.price) {
            ...
            kesaiList +:= price.price - order.price
            b.break
          } else if (order.price < price.price && isClosingTime(price.trdTime)) {
            kesaiList +:= price.price - order.price - 5
            b.break
          }
        }
      }
      ...
      b.breakable {
        for (price <- priceList.drop(sellOrder.index + 1)) {
          if (order.price - sellOrder.rieki > price.price) {
            ...
            kesaiList +:= order.price - price.price - 5
            b.break
          }
        }
      }
